...
class IProductRepository(ABC):
    ...
    def get_by_category(self, category: str) -> List[Product]:
        ...
        sol:
        obtiene todos los productos con get_all(), despues itera la lista,
        si encuentra un match lo agrega a una lista de salida y 
        si no encuentra un match con la categoria (out esta vacia)
        reporta con print y retorna None 
        """
        pass
        all = self.get_all()
        ...
        if out.__len__() > 0:
            ...
            print("Categoría de producto no encontrada")
            return None
    ...
    def save(self, product: Product) -> Product:
        ...
        sol:
        detecta si elproducto no contiene una ID, sino, crea uno nuevo y reporta
        Si el producto tiene ID, esta es actualizada y reporta
        """
        pass
        if product.id is None:
            print("Creando nuevo ID para producto")
        else:
            product.id = product.id ### check on future where does new id gonna come from, user interface maybe?
            print(f"Actualizando producto con ID : {product.id}")
        return product
    ...
    def delete(self, product_id: int) -> bool:
        ...
        sol:
        obtiene todos los productios con get_all(), despues itera la lista, si encuentra un producto con el ID
        elimina y reporta si lo encuentra, sino, reporta y arroja false
        """
        pass
        all = self.get_all()
        for product in all:
